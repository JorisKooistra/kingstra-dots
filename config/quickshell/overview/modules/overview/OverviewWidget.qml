...
Item {
    ...
    function rememberWorkspaceDropPosition(workspaceId, localX, localY) {
    ...
    function moveWindowToWorkspaceDrop(windowItem, targetWorkspace) {
        ...
        const dropX = root.clamp(root.draggingTargetWorkspaceX, 0, root.workspaceImplicitWidth);
        const dropY = root.clamp(root.draggingTargetWorkspaceY, 0, root.workspaceImplicitHeight);
        const maxTopLeftX = Math.max(0, root.workspaceImplicitWidth - windowItem.width);
        const maxTopLeftY = Math.max(0, root.workspaceImplicitHeight - windowItem.height);
        const topLeftX = root.clamp(dropX - (windowItem.Drag.hotSpot.x || 0), 0, maxTopLeftX);
        const topLeftY = root.clamp(dropY - (windowItem.Drag.hotSpot.y || 0), 0, maxTopLeftY);

        const cursorX = Math.round(baseX + dropX / scale);
        const cursorY = Math.round(baseY + dropY / scale);
        ...
        Quickshell.execDetached([
            "bash",
            Quickshell.env("HOME") + "/.config/hypr/scripts/overview-move-window.sh",
            `${windowItem.windowData.address}`,
            `${targetWorkspace}`,
            `${cursorX}`,
            `${cursorY}`,
            `${windowX}`,
                    ...
                    MouseArea {
                        ...
                        onReleased: {
                            ...
                            if (targetSpecialWorkspace === root.createSpecialWorkspaceTarget) {
                                ...
                                root.moveWindowToWorkspaceDrop(window, targetWorkspace)
                                updateWindowPosition.restart()
                            }
                            else {
