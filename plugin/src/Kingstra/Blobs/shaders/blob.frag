...
// De gradient loopt over de hele groep (het scherm), niet over het item. Met
// qt_TexCoord0 kreeg elke blob zijn eigen volledige kleurverloop, waardoor een
// paneel niet aansloot op de omlijsting eromheen. Door op de group-space
// pixelpositie te sampelen leest alles als één doorlopend verloop.
// Projecteer op een richtingsvector in plaats van op één as: zo kan het
// verloop ook diagonaal lopen. Lijnen loodrecht op die richting houden dan
// exact dezelfde kleur, terwijl je langs de richting van kleur a naar b gaat.
float gradientT(vec2 pixel) {
    float p = dot(pixel, vec2(gradientDirX, gradientDirY));
    return clamp((p - gradientOrigin) / max(gradientSpan, 1.0), 0.0, 1.0);
}

vec4 sampleGradient(vec4 a, vec4 b, vec4 c, float t) {
    return t < 0.5 ? mix(a, b, t * 2.0) : mix(b, c, (t - 0.5) * 2.0);
}

vec4 sampleFillGradient(vec2 pixel) {
    return sampleGradient(gradientStart, gradientMid, gradientEnd, gradientT(pixel));
}

vec4 sampleBorderGradient(vec2 pixel) {
    return sampleGradient(borderStart, borderMid, borderEnd, gradientT(pixel));
}
...
void main() {
    ...
    // Accentrand als band van borderWidth net binnen de contour, op de
    // gemeenschappelijke SDF zodat hij de samengesmolten vorm volgt — ook
    // voor de omlijsting zelf (myIndex == -1). Frame en panelen delen één
    // SDF, dus dit is per definitie één doorlopende lijn: waar een paneel
    // met de omlijsting samensmelt verdwijnt de band vanzelf in de merge
    // (mergedSdf diep negatief) en loopt hij aan de buitenkant om de
    // samengestelde vorm heen. Elke QML-outline hierbovenop tekent
    // onvermijdelijk een tweede, afwijkende lijn.
    float bandT = borderWidth > 0.0
        ? smoothstep(-fw, fw, mergedSdf + borderWidth)
        : 0.0;
    vec4 col = mix(sampleFillGradient(pixel), sampleBorderGradient(pixel), bandT);

    float a = alpha * col.a;
    fragColor = vec4(col.rgb * a, a) * qt_Opacity;
}
